...
Content     :   D3D11 application/Window setup functionality for RoomTiny(OculusTouchSteamLink).
...
#ifndef VALIDATE
#define VALIDATE(x, msg) if (!(x)) { MessageBoxA(nullptr, (msg), "OculusTouchSteamLink", MB_ICONERROR | MB_OK); exit(-1); }
#endif

#ifndef FATALERROR
#define FATALERROR(msg) { MessageBoxA(nullptr, (msg), "OculusTouchSteamLink", MB_ICONERROR | MB_OK); exit(-1); }
#endif
...
struct DirectX11
{
    ...
    //std::shared_ptr<OculusTouchSteamLink::IVRDriver> _driver;

    void Run(
        bool (*MainLoop)(
            bool retryCreate,
            std::shared_ptr<OculusTouchSteamLink::IVRDriver> driver,
            ovrSession session,

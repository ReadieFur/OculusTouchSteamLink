...
static std::shared_ptr<OculusTouchSteamLink::IVRDriver> driver;
...
void* HmdDriverFactory(const char* interface_name, int* return_code) {
	if (std::strcmp(interface_name, vr::IServerTrackedDeviceProvider_Version) == 0) {
		if (!driver) {
			...
			driver = std::make_shared<OculusTouchSteamLink::VRDriver>();
		}
...
std::shared_ptr<OculusTouchSteamLink::IVRDriver> OculusTouchSteamLink::GetDriver() {
	return driver;
}
